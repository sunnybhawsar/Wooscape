      ...
      <v-card>
        ...
        <v-card-text>
          <v-container grid-list-md>
            <v-layout wrap>
              <v-flex xs12 sm6 md4>
                <v-text-field v-model="editedItem.name" label="Fish name"></v-text-field>
              </v-flex>
              <v-flex xs12 sm6 md4>
                <v-text-field v-model="editedItem.desc" label="Fish Desc"></v-text-field>
              </v-flex>
              <v-flex xs12 sm6 md4>
   ...
   if(this.$store.state.isUserLoggedIn && this.$store.state.user.isAdmin){
    ...
    methods: {
      async initialize () {
        ...
        this.fishes = fishes.data
      },

      editItem (item) {
        this.editedIndex = item.id
        this.arrayIndex = this.fishes.indexOf(item)
      ...
      async deleteItem (item) {
        const sindex = this.fishes.indexOf(item)
        var flag = null
        confirm('Are you sure you want to delete this item?') && (flag = await FishesService.delete(item.id))
        console.log(flag)
        if(flag) {
          this.fishes.splice(sindex, 1)
        }
      },
      ...
      async save () {
        if (this.editedIndex > -1) {
          const editedFish = await FishesService.put(this.editedItem.id, this.editedItem)
          console.log(editedFish.data)
          Object.assign(this.fishes[this.arrayIndex], editedFish.data)
        } else {
          const newFish = await FishesService.post(this.editedItem)
          console.log(newFish.data)
          this.fishes.push(newFish.data)
        }
        this.close()
      },
