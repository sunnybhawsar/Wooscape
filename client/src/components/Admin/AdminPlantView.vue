      ...
      <v-card>
        ...
        <v-card-text>
          <v-container grid-list-md>
            <v-layout wrap>
              <v-flex xs12 sm6 md4>
                <v-text-field v-model="editedItem.name" label="Plant name"></v-text-field>
              </v-flex>
              <v-flex xs12 sm6 md4>
                <v-text-field v-model="editedItem.desc" label="Plant Desc"></v-text-field>
              </v-flex>
              <v-flex xs12 sm6 md4>
   ...
   if(this.$store.state.isUserLoggedIn && this.$store.state.user.isAdmin){
    ...
    methods: {
      ...
      editItem (item) {
        ...
        this.arrayIndex = this.plants.indexOf(item)
      ...
      async deleteItem (item) {
        const sindex = this.plants.indexOf(item)
        var flag = null
        confirm('Are you sure you want to delete this item?') && (flag = await PlantsService.delete(item.id))
        console.log(flag)
        if(flag) {
          this.plants.splice(sindex, 1)
        }
      },
      ...
      async save () {
        if (this.editedIndex > -1) {
          const editedPlant = await PlantsService.put(this.editedItem.id, this.editedItem)
          console.log(editedPlant.data)
          Object.assign(this.plants[this.arrayIndex], editedPlant.data)
        } else {
          const newPlant = await PlantsService.post(this.editedItem)
          console.log(newPlant.data)
          this.plants.push(newPlant.data)
        }
        this.close()
      },
